...
\usepackage{hyperref}          % PDF元数据
\usepackage{draftwatermark}    % 文字水印（稳定可靠）
\usepackage{pifont}            % 提供带圈数字（如需使用 \ding{172} 等）

% -------------------- 版权信息定义 --------------------
\newcommand{\copyrightYear}{2026}
\newcommand{\copyrightOwner}{Huang Kunlun}
\newcommand{\licenseText}{本作品采用 CC BY 4.0 许可证（详见 \url{https://creativecommons.org/licenses/by/4.0/}）。}
\newcommand{\githuburl}{\url{https://github.com/hkwk/formula-in-metrology/}}

% -------------------- 设置PDF元数据（无 pdfcopyright） --------------------
\hypersetup{
    pdftitle={注册计量师公式汇编},
    pdfauthor={\copyrightOwner},
    pdfsubject={计量学；版权声明：\copyright\ \copyrightYear\ \copyrightOwner},
    pdfkeywords={注册计量师, 测量不确定度, 统计},
    pdfcreator={XeLaTeX},
    % 若需额外版权信息，可放在 pdfsubject 中
}

% -------------------- 设置页眉页脚 --------------------
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small \copyright\ \copyrightYear\ \copyrightOwner\ \githuburl}
\fancyhead[R]{\small 注册计量师公式}
\fancyfoot[C]{\thepage\ / \pageref{LastPage}}
\renewcommand{\headrulewidth}{0.4pt}
\renewcommand{\footrulewidth}{0pt}

% -------------------- 添加文字水印（使用 draftwatermark） --------------------
\SetWatermarkText{\copyright\ \copyrightYear\ \copyrightOwner }
\SetWatermarkLightness{0.85}      % 0~1，数值越大颜色越淡（1=白色）
\SetWatermarkAngle{45}            % 倾斜角度
\SetWatermarkScale{0.6}             % 字号缩放
...
\begin{center}
    \vspace*{\fill}
    {\Huge \copyright\ \copyrightYear\ \copyrightOwner} \\[1cm]
    {\Large 保留所有权利。} \\[0.5cm]
    \licenseText \\[0.5cm]
    {\large 您可以自由地分享（复制、分发）和改编（修改、衍生）本作品，但必须注明出处。} \\[0.5cm]
    {\large Original version is from \githuburl} \\
    \vspace*{\fill}
\end{center}
\newpage
...
% ---------- 以下保留您的全部原有内容（从 \section{注册计量师公式} 开始） ----------
\section{注册计量师公式}
...
\begin{table}[!ht]
    ...
    \hline
        \(\Delta[u(x_i)]/u(x_i)\) & 0 & 0.10 & 0.20 & 0.25 & 0.50 \\ \hline
        \(\nu_i\) & \(\infty\) & 50 & 12 & 8 & 2 \\ \hline
    \end{tabular}
\end{table}
...
\begin{table}[!ht]
    ...
    \hline
        \(p\) & 0.50 & 0.68 & 0.90 & 0.95 & 0.9545 & 0.99 & 0.9973 \\ \hline
        \(k\) & 0.675 & 1 & 1.645 & 1.960 & 2 & 2.576 & 3 \\ \hline
    \end{tabular}
\end{table}
...
(4) 梯形分布 \(\sqrt{\frac{6}{1+\beta^2}}\)（\(\beta\) 为梯形上底半宽度与下底半宽度之比，当 \(\beta\) 等于 1 时，梯形分布变矩形分布；当 \(\beta\) 等于 0 时，变为三角分布）；
...
对于某些计量标准，如量块，其检定规程规定其扩展不确定度对应于99\%的包含概率，此时所给出的扩展不确定度所对应的k值与2相差较大。在进行判断时，应先将其换算到对应于k=2时的扩展不确定度。
